...
TEST(ScannerTest, KeywordRecognition) {
    ...
    const std::string code = "int float double char void if else for while do return break continue "
...
TEST(ScannerTest, IdentifierRecognition) {
    const Scanner scanner;
    const std::string code = "var1 _var2 Var3 var_with_underscores 123invalid";
...
TEST(ScannerTest, IntegerRecognition) {
    const Scanner scanner;
    const std::string code = "123 0x1A 0XfF 0123 0 123u 456U 789l 0L 123ul";
...
TEST(ScannerTest, FloatRecognition) {
    const Scanner scanner;
    const std::string code = "123.45 67. .89 10e5 20E-3 30.45f 50.67F 70.89l 90.0L";
...
TEST(ScannerTest, StringRecognition) {
    const Scanner scanner;
    const std::string code = R"("hello" "line1\nline2" "with \"quote\"" "empty")";
...
TEST(ScannerTest, CommentRecognition) {
    ...
    // 检查注释
    EXPECT_EQ(tokens[0].type, TokenType::TOK_COMMENT);
    EXPECT_EQ(tokens[3].type, TokenType::TOK_COMMENT);
    EXPECT_EQ(tokens[6].type, TokenType::TOK_COMMENT);
}

// 测试未闭合字符串错误
TEST(ScannerTest, IncompleteStringError) {
    const Scanner scanner;
    const std::string code = "\"未闭合的字符串";

    const auto [tokens, errors] = scanner.scan(code);

    EXPECT_EQ(errors.size(), 1);
    EXPECT_EQ(errors[0].type, ErrorType::INCOMPLETE_STRING);
}

// 测试未闭合字符错误
TEST(ScannerTest, IncompleteCharError) {
    const Scanner scanner;
    const std::string code = "'未闭合的字符";

    const auto [tokens, errors] = scanner.scan(code);

    EXPECT_EQ(errors.size(), 1);
    EXPECT_EQ(errors[0].type, ErrorType::INCOMPLETE_CHAR);
}

// 测试非法转义序列错误
TEST(ScannerTest, IllegalEscapeError) {
    const Scanner scanner;
    const std::string code = "\"非法转义: \\a \\z\" '\\q'";

    const auto [tokens, errors] = scanner.scan(code);

    // 应该有两个非法转义错误
    EXPECT_EQ(errors.size(), 2);
    EXPECT_EQ(errors[0].type, ErrorType::ILLEGAL_ESCAPE);
    EXPECT_EQ(errors[1].type, ErrorType::ILLEGAL_ESCAPE);
}

// 测试未闭合多行注释错误
TEST(ScannerTest, IncompleteCommentError) {
    const Scanner scanner;
    const std::string code = "int x; /* 未闭合的多行注释\n"
            "这是第二行";

    const auto [tokens, errors] = scanner.scan(code);

    EXPECT_EQ(errors.size(), 1);
    EXPECT_EQ(errors[0].type, ErrorType::INCOMPLETE_COMMENT);
}

// 测试无效字符错误
TEST(ScannerTest, InvalidCharacterError) {
    const Scanner scanner;
    const std::string code = "int x = 10; $ %";

    const auto [tokens, errors] = scanner.scan(code);

    // $和%是无效字符
    EXPECT_EQ(errors.size(), 2);
    EXPECT_EQ(errors[0].type, ErrorType::INVALID_CHARACTER);
    EXPECT_EQ(errors[1].type, ErrorType::INVALID_CHARACTER);
}

// 测试复杂代码片段
TEST(ScannerTest, ComplexCodeFragment) {
    const Scanner scanner;
    const std::string code = "/* 计算斐波那契数列 */\n"
            "int fibonacci(int n) {\n"
            "    if (n <= 1)\n"
            "        return n;\n"
